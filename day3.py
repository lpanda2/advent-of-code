...
def split_wires(wire_paths):
    return wire_paths.split('\n')


def make_lines(wire_point_pairs, name):
    lc = mc.LineCollection(wire_point_pairs[0], color='g', linewidths=1)
    lc2 = mc.LineCollection(wire_point_pairs[1], color='b', linewidths=1)
    fig, ax = pl.subplots()
...
def pairs(wire):
    lp = []
    for i in list(range(len(wire))):
        if i == len(wire) - 1:
            continue
        lp.append((wire[i], wire[i+1]))
    return lp
    

def find_intersection(line_1, line_2):
    try:
        line_1_1,line_1_2 = line_1
        line_2_1,line_2_2 = line_2
        x1,y1 = line_1_1
        x2,y2 = line_1_2
        x3,y3 = line_2_1
        x4,y4 = line_2_2
        ...
        if (px > max(x1,x2)) or (px > max(x3,x4)) or (px < min(x1,x2)) or (px < min(x3,x4)):
            return None
        if (py > max(y1,y2)) or (py > max(y3,y4)) or (py < min(y1,y2)) or (py < min(y3,y4)):
            return None
...
def find_intersection_with_distance(line_1, line_2):
    try:
        line_1, dist1 = line_1
        line_2, dist2 = line_2
        line_1_1,line_1_2 = line_1
        ...
        return [px, py, dist1+dist2 - abs(x2-px) - abs(x4-px) - abs(y2-py) - abs(y4-py)]
    except ZeroDivisionError:
        return None
